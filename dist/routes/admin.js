...
admin.use("*", admin_auth_1.adminAuthMiddleware);
...
admin.get("/dashboard/overview", async (c) => {
    const [totalsResult, statusResult, recentLeadsResult, monthlyLeadsResult] = await Promise.all([
        client_1.pool.query(`
        SELECT
...
admin.get("/restaurants", async (c) => {
  ...
  `;
    const [listResult, countResult] = await Promise.all([
        client_1.pool.query(listQuery, [...params, pageSize, offset]),
        client_1.pool.query(countQuery, params),
    ]);
    return c.json({
...
admin.post("/restaurants", async (c) => {
    ...
    try {
        ...
        let slug = (0, slugify_1.default)(restaurantName, {
            lower: true,
            strict: true,
        });
        const slugCheck = await client.query(`SELECT id FROM restaurants WHERE slug = $1 LIMIT 1`, [slug]);
      ...
      `, [restaurantName, slug, "pro", googleReviewUrl]);
        ...
        const passwordHash = await bcrypt_1.default.hash(ownerPassword, 10);
...
admin.put("/restaurant-users/:userId", async (c) => {
    ...
    const existingResult = await client_1.pool.query(`
...
admin.get("/demo-requests", async (c) => {
    ...
    if (search) {
        params.push(`%${search}%`);
        conditions.push(`(dr.name ILIKE $${params.length}
...
admin.put("/demo-requests/:id", async (c) => {
    ...
    if (status && !VALID_DEMO_REQUEST_STATUSES.has(status)) {
    ...
    const firstContactExpression = markContacted && !existingLead.first_contact_at ? "NOW()" : "first_contact_at";
    const result = await client_1.pool.query(`
    UPDATE demo_requests
    SET status = $1,
        priority = $2,
